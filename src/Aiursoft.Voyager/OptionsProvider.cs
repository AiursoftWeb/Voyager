...
{
    public static readonly Option<string> PathOption = new(name: "--path")
    {
    ...
    {
        Description =
            "The short name of the template to use. Run `voyager list` to see all available templates.",
        Required = true,
    };
    ...
    {
        DefaultValueFactory = _ =>
            "https://github.com/AiursoftWeb/Voyager/raw/refs/heads/master/templates.json",
    ...
    )
    {
        DefaultValueFactory = _ =>
        {
            ...
            Console.WriteLine(
                $"Didn't pass the argument '-n', using the current directory name '{name}' as the project name."
            );
            return name;
        },
